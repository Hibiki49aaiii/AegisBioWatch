...
TARGETS = [
    {
        "id": "C301_TO_ACCEPTED_1V8_RAIL",
        "net": "+1V8",
        "a": "Pad 1 [+1V8] of C301 on Top_layer",
        "b": "Track [+1V8] on Top_layer, length 4.5000 mm",
    },
]!/usr/bin/env python3
"""Read-only optimized coarse multi-bend screen for low-current route-1bk candidates."""
from __future__ import annotations
...
def main() -> None:
    ...
    for ref,val in expected_values.items():
        if ref not in fps or fps[ref].GetValue()!=val:
            raise SystemExit(f"route1bl C301 coarse identity gate failed: {ref}")
    ...
    if out["board_modified"] is not False:
        raise SystemExit("route1bl C301 coarse board-modified gate failed")
