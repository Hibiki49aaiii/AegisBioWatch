#!/usr/bin/env python3
"""Read-only local 0.05 mm refine for route-1bl VSYS -> R106.1."""
...
def main() -> None:
    ...
    xs=frange(X_MIN,X_MAX,GRID)
    ys=frange(Y_MIN,Y_MAX,GRID)

    raw=[]
    for x in xs:
        for y in ys:
            pts=normalize([A,(x,A[1]),(x,y),(B[0],y),B])
            length=path_length(pts)
            if len(pts)==5 and length<=MAX_PATH_LENGTH+EPS:
                raw.append((4,round(length,6),"HVHV",pts))
    ...
    all_results.sort(key=lambda p:(-p["minimum_conservative_clearance_mm"],p["path_length_mm"],p["points_mm"]))
    passing.sort(key=lambda p:(-p["minimum_conservative_clearance_mm"],p["path_length_mm"],p["points_mm"]))
    best_clearance=all_results[0] if all_results else None
    out={
        "revision":"r13-route1bl-r106-vsys-local-refine",
        ...
        "top_clearance_paths":all_results[:20],
