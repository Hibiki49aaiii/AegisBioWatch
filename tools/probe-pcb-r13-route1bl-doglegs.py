...
EXCLUDED_NETS = {
    ...
    "PMIC_SW1", "PMIC_SW2", "PVSS1_LOCAL", "SYS_I2C_SCL", "DOCK_5V_RAW",
}
...
def main() -> None:
    ...
    if report.get("output_sha256") != src_sha:
        raise SystemExit("route1bl dogleg source SHA gate failed")
    ...
    if len(drc.get("violations", [])) != 0 or len(drc.get("unconnected_items", [])) != 113:
        raise SystemExit("route1bl dogleg DRC source gate failed")
    if audit.get("result") != "PASS" or audit.get("audited_present_source_nodes") != 268:
        raise SystemExit("route1bl dogleg audit source gate failed")
    ...
    out = {
        "revision": "r13-route1bl-manhattan-dogleg-screen",
    ...
    if out["board_modified"] is not False or out["raw_pair_count"] < 100:
        raise SystemExit("route1bl dogleg state gate failed")
