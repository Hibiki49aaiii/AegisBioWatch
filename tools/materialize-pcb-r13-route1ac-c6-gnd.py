#!/usr/bin/env python3
"""r13 route-1ac: close C6.2/GND directly into continuous In1.Cu GND.

Source: executed-KiCad-clean route-1ab (0 violations / 147 unconnected /
268-node physical audit PASS). C6 is the 2.2 uF NRF_DECD decoupler. This
increment gives C6 its own short local ground return instead of daisy-chaining
through another decoupling capacitor. NRF_DECD remains untouched.
"""
...
def main():
    ...
    c6=fps.get('C6')
    if c6 is None: raise SystemExit('route1ac missing C6')
    gates={'C6.1':pads(c6,'1')[0].GetNetname(),'C6.2':pads(c6,'2')[0].GetNetname(),'C6.value':c6.GetValue()}
    expected={'C6.1':'NRF_DECD','C6.2':'GND','C6.value':'2.2uF'}
    if gates!=expected: raise SystemExit(f'route1ac C6 net/value gate failed: {gates}')
    gnd_pad=point(c6,'2')
    if abs(gnd_pad[0]-12.145)>0.001 or abs(gnd_pad[1]-22.225)>0.001: raise SystemExit(f'route1ac C6.2 geometry gate failed: {gnd_pad}')
