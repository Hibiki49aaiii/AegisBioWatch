...
def main() -> None:
    ...
    if report.get("output_sha256") != src_sha:
        raise SystemExit("route1bj dogleg source SHA gate failed")
    ...
    if len(drc.get("violations", [])) != 0 or len(drc.get("unconnected_items", [])) != 115:
        raise SystemExit("route1bj dogleg DRC source gate failed")
    if audit.get("result") != "PASS" or audit.get("audited_present_source_nodes") != 268:
        raise SystemExit("route1bj dogleg audit source gate failed")
    ...
    out = {
        "revision": "r13-route1bj-manhattan-dogleg-screen",
        "source_route1bi_sha256": src_sha,
        "source_gate": {
            "rule_violations": 0,
    ...
    if out["board_modified"] is not False or out["raw_pair_count"] < 100:
        raise SystemExit("route1bj dogleg state gate failed")
