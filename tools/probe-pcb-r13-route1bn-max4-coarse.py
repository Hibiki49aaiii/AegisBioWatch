#!/usr/bin/env python3
"""Read-only max-four-segment coarse screen for ordinary route-1bm ratsnest pairs."""
...
EXCLUDED_NETS = {
    ...
    "PMIC_SW1", "PMIC_SW2", "PVSS1_LOCAL", "SYS_I2C_SCL", "DOCK_5V_RAW", "NRF_RESET_N",
}
...
def main() -> None:
    ...
    ap.add_argument("--route1bm-drc-json", "--drc-json", dest="drc_json", required=True)
    ...
    if report.get("output_sha256") != src_sha:
        raise SystemExit("route1bn coarse source SHA gate failed")
    ...
    if len(drc.get("violations", [])) != 0 or len(drc.get("unconnected_items", [])) != args.expected_unconnected:
        raise SystemExit("route1bn coarse DRC source gate failed")
    if audit.get("result") != "PASS" or audit.get("audited_present_source_nodes") != 268:
        raise SystemExit("route1bn coarse audit source gate failed")
    ...
    for c in raw:
        ...
        for lane_x in xs:
            for lane_y in ys:
                ...
                add_path("VHVH", [A, (A[0], lane_y), (lane_x, lane_y), (lane_x, B[1]), B])
    ...
    out = {
        "revision": "r13-route1bn-max4-coarse-screen",
        "source_route1bm_sha256": src_sha,
    ...
    if out["board_modified"] is not False or out["raw_pair_count"] < 100:
        raise SystemExit("route1bn max4 coarse state gate failed")
